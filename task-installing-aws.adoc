...
. Wenn Sie einen AWS-Zugriffsschlüssel eingegeben haben, wählen Sie einen Speicherort für die Instanz aus, wählen ein Schlüsselpaar aus, wählen Sie aus, ob eine öffentliche IP-Adresse aktiviert werden soll, und wählen Sie eine vorhandene IAM-Rolle aus, oder lassen Sie das Feld leer, damit Cloud Sync die Rolle für Sie erstellt. Sie haben auch die Möglichkeit, Ihren Daten-Broker mit einem KMS-Schlüssel zu verschlüsseln.
+
...
VCPUs:: 4
RAM:: 16 GB
Betriebssystem:: Amazon Linux 2022
